...
import DMUpdatePriorityModal from '../DMUpdatePriorityModal';
import DMUpdateDeliveryWindow from '../DMUpdateDeliveryWindow';
import styles from './DMTableStyles';
  ...
  (
    ...
    tableSettings[TASK_FIELDS_NAME.TASK_NUMBER] && {
      name: (
        <input
          ...
          checked={selectedCount !== 0 && selectedCount === realDataSet.length}
          onChange={() => {
            handleSelectAllToggle(dataSet);
          }}
          className={`${css(styles.checkboxTop)}`}
        />
      ),
    ...
    tableSettings[TASK_FIELDS_NAME.STATUS] && {
      ...
      cell: row => {
        ...
        const isPriority = row[TASK_FIELDS_NAME.PRIORITY] === 1;
    ...
    tableSettings[TASK_FIELDS_NAME.ORDERVALUE] && {
      name: tableSettings[TASK_FIELDS_NAME.ORDERVALUE].name,
      selector: tableSettings[TASK_FIELDS_NAME.ORDERVALUE].selector,
      sortable: tableSettings[TASK_FIELDS_NAME.ORDERVALUE].sortable,
      minWidth: tableSettings[TASK_FIELDS_NAME.ORDERVALUE].minWidth,
      omit: tableSettings[TASK_FIELDS_NAME.ORDERVALUE].visible,
      cell: row => {
        return <p>{row[TASK_FIELDS_NAME.ORDERVALUE]}</p>;
      }
    }
  ]
);
    ...
    return (
      <div className={`${css(AppStyles.flex1, styles.wrapper)}`}>
        <DMTableHeader
          ...
          onChangePriorityPress={this.props.onChangePriorityPress}
        ...
        {!_.isNull(this.props.tasksListForChangePriority) && (
          <DMUpdatePriorityModal
            tasksList={this.props.tasksListForChangePriority}
            onModalCloseClick={this.props.onDMUpdatePriorityModalCloseClick}
          />
        )}
        {/* {true && ( */}
