...
class DMTableController extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tasksListForChangePriority: null,
  ...
  onChangePriorityPress = () => {
    this.setState({
      tasksListForChangePriority: _.map(
        this.getAllSelectedTasks(),
        TASK_FIELDS_NAME.TASK_NUMBER
      )
    });
  ...
  onDMUpdatePriorityModalCloseClick = () => {
    this.setState({
      tasksListForChangePriority: null
    });
  };
  ...
  convertArrayOfObjectsToCSV = array => {
    ...
    array.forEach(item => {
      item[TASK_FIELDS_NAME.CREATED_AT] = _.isEmpty(
        item[TASK_FIELDS_NAME.CREATED_AT]
      )
        ? '-'
        : moment(item[TASK_FIELDS_NAME.CREATED_AT]).format(
            DATE_TIME_FORMAT_EXPORT
          );

      item[TASK_FIELDS_NAME.COMPLETE_AFTER] = _.isEmpty(
        item[TASK_FIELDS_NAME.COMPLETE_AFTER]
      )
        ? '-'
        : moment(item[TASK_FIELDS_NAME.COMPLETE_AFTER]).format(
            DATE_TIME_FORMAT_EXPORT
          );

      item[TASK_FIELDS_NAME.COMPLETE_BEFORE] = _.isEmpty(
        item[TASK_FIELDS_NAME.COMPLETE_BEFORE]
      )
        ? '-'
        : moment(item[TASK_FIELDS_NAME.COMPLETE_BEFORE]).format(
            DATE_TIME_FORMAT_EXPORT
          );

      item[TASK_FIELDS_NAME.END_TIME] = _.isEmpty(
        item[TASK_FIELDS_NAME.END_TIME]
      )
        ? '-'
        : moment(item[TASK_FIELDS_NAME.END_TIME]).format(
            DATE_TIME_FORMAT_EXPORT
          );

      item[TASK_FIELDS_NAME.UPDATED_AT] = _.isEmpty(
        item[TASK_FIELDS_NAME.UPDATED_AT]
      )
        ? '-'
        : moment(item[TASK_FIELDS_NAME.UPDATED_AT]).format(
            DATE_TIME_FORMAT_EXPORT
          );

      item[TASK_FIELDS_NAME.ETA] = _.isNil(item[TASK_FIELDS_NAME.ETA])
        ? '-'
        : moment(item[TASK_FIELDS_NAME.ETA]).format(DATE_TIME_FORMAT_EXPORT);

        item[TASK_FIELDS_NAME.ORDERVALUE] = _.isNil(item[TASK_FIELDS_NAME.ORDERVALUE])
        ? '-' :item[TASK_FIELDS_NAME.ORDERVALUE];
  ...
  render() {
    ...
    return (
      <DMTableView
        ...
        onChangePriorityPress={this.onChangePriorityPress}
        ...
        onDMUpdatePriorityModalCloseClick={
          this.onDMUpdatePriorityModalCloseClick
        }
        tasksListForChangeAssigment={tasksListForChangeAssigment}
        tasksListForChangePriority={tasksListForChangePriority}
